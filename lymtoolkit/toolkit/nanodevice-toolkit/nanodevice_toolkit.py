...
def render_hemt_component(scene, values, visible_layers=None):
    ...
    preview_buckets = {
        ...
        "fine_source_drain": buckets["source_fine"] + buckets["drain_fine"],
        "top_gate": buckets["gate"],
        "fine_top_gate": buckets["gate_fine"],
    ...
    style_keys = {
        "source_drain": "source",
        "fine_source_drain": "source_fine",
        "top_gate": "gate",
        "fine_top_gate": "gate_fine",
...
HEMT_COMPONENT_TOOL = ToolSpec(
    ...
    preview_layers=[
        ...
        ("fine_source_drain", "Fine Source / Drain (26)"),
        ("top_dielectric", "Gate Dielectric"),
        ("top_gate", "Gate"),
        ("fine_top_gate", "Fine Gate (28)"),
        ("labels", "Labels"),
    ...
    params=[
        ...
        ParameterSpec("gate_pad_retraction", "Gate Pad Retraction / Gap", "GPr", "Gate", 1.0, minimum=0.0, maximum=5000.0, suffix=" um", tooltip="Double-finger only: retract the right edge of the left-side gate pad/manifold to create clearance from the fixed centre drain contact."),
        ParameterSpec("source_gate_spacing", "Source-Gate Spacing", "Lsg", "Access", 1.5, minimum=0.0, maximum=1000.0, suffix=" um", tooltip="Edge-to-edge source/gate spacing before Source-Gate Overlap is subtracted; may be zero."),
        ParameterSpec("gate_drain_spacing", "Gate-Drain Spacing", "Lgd", "Access", 2.0, minimum=0.0, maximum=1000.0, suffix=" um", tooltip="Edge-to-edge gate/drain spacing before Gate-Drain Overlap is subtracted; may be zero."),
        ParameterSpec("source_gate_overlap", "Source-Gate Overlap", "Osg", "Access", 0.0, minimum=0.0, maximum=1000.0, suffix=" um", tooltip="Extends only the Gate edge facing Source; Source stays fixed. The signed edge gap is Lsg minus Osg."),
        ParameterSpec("gate_drain_overlap", "Gate-Drain Overlap", "Ogd", "Access", 0.0, minimum=0.0, maximum=1000.0, suffix=" um", tooltip="Extends only the Gate edge facing Drain; Drain stays fixed. The signed edge gap is Lgd minus Ogd."),
        ...
        ParameterSpec("split_ebl_exposure", "Split Fine / Coarse EBL", "EBL", "EBL Split", 0, kind="choice", choices=[("false", False), ("true", True)], tooltip="Move the HEMT core S/D and gate structures to corresponding layers 26 and 28 while keeping outer metal on layers 16 and 18."),
        ParameterSpec("fine_fanout_depth", "Fine U Depth", "Fdep", "EBL Split", 12.0, minimum=0.0, maximum=5000.0, suffix=" um", tooltip="How far the open-ended fine-layer U arms extend from the inner source/drain contact towards the coarse fanout."),
        ParameterSpec("fine_fanout_width", "Fine S/D U Line Width", "Fwid", "EBL Split", 1.0, minimum=0.05, maximum=5000.0, suffix=" um", tooltip="Shared line width of the Source and Drain fine inner-pad bases and every U-shaped arm. For a double-finger centre Drain, 2 x Fwid must be smaller than Wohm."),
        ParameterSpec("ebl_overlap", "Fine / Coarse Overlap", "Ovl", "EBL Split", 2.0, minimum=0.0, maximum=5000.0, suffix=" um", tooltip="Overlap length between fine and coarse exposure geometry at source/drain, drain, and gate transitions."),
...
WRITE_READ_ARRAY_TOOL = ToolSpec(
    ...
    params=[
        ...
        ParameterSpec("fet_gap", "FET Gap", "Gap", "Routing", 4.0, minimum=0.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("shared_contact_height", "Shared Height", "Sch", "Routing", 10.0, minimum=2.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("outer_contact_min_length", "Outer Contact Min", "Ocm", "Routing", 4.0, minimum=2.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("write_width", "Write Width", "Ww", "Write FET", 30.0, minimum=2.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("write_length", "Write Length", "Lw", "Write FET", 5.0, minimum=2.0, maximum=1000.0, suffix=" um"),
        ...
        ParameterSpec("write_left_contact_length", "Write Left Contact", "Wlc", "Write FET", 10.0, minimum=2.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("write_right_contact_length", "Write Right Contact", "Wrc", "Write FET", 6.0, minimum=2.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("read_width", "Read Width", "Wr", "Read FET", 13.0, minimum=2.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("read_length", "Read Length", "Lr", "Read FET", 13.5, minimum=2.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("read_channel_overlap", "Read Channel Overlap", "Rco", "Read FET", 5.0, minimum=0.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("read_pad_overhang", "Read Pad Overhang", "Rpo", "Read FET", 1.0, minimum=0.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("read_contact_length", "Read Contact Length", "Rcl", "Read FET", 10.0, minimum=2.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("gate_extension", "Gate Extension", "Gex", "Gate", 2.0, minimum=0.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("gate_length_overhang", "Gate Length Overhang", "Glo", "Gate", 4.0, minimum=0.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("gate_width_overhang", "Gate Width Overhang", "Gwo", "Gate", 2.0, minimum=0.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("gate_gap", "Gate Gap", "Gg", "Gate", 3.0, minimum=0.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("write_gate_x_offset", "Write Gate X Offset", "Wgx", "Gate", -1.5, minimum=-1000.0, maximum=1000.0, suffix=" um", tooltip="Shifts the write FET gate horizontally without changing the transistor body."),
        ParameterSpec("coupling_gate_pad_size", "Coupling Via Pad", "Cgp", "Coupling", 15.0, minimum=2.0, maximum=1000.0, suffix=" um", tooltip="Base size used for bottom-gate coupling features and the read bottom stub."),
        ParameterSpec("coupling_via_size", "Dielectric Via Size", "Cvs", "Coupling", 2.0, minimum=1.0, maximum=1000.0, suffix=" um"),
        ParameterSpec("read_gate_bridge_width", "Read Gate Bridge", "Rgb", "Coupling", 12.0, minimum=1.0, maximum=1000.0, suffix=" um"),
        ...
        ParameterSpec("read_bottom_overlap_height", "Read Bottom Overlap", "Rbo", "Coupling", 10.0, minimum=0.0, maximum=1000.0, suffix=" um", tooltip="Overlap height between the read bottom contact and the upward RWL gate stub."),
        ParameterSpec("write_coupling_via_inset", "Write Via Inset", "Wvi", "Coupling", 1.0, minimum=0.0, maximum=1000.0, suffix=" um", tooltip="Keeps the write-side coupling via inside the contact-only overlap region, away from the channel."),
        ParameterSpec("draw_channel", "Draw Channel", "Ch", "Options", 1, kind="choice", choices=[("true", True), ("false", False)]),
        ParameterSpec("draw_top_dielectric", "Top Dielectric", "Td", "Options", 0, kind="choice", choices=[("true", True), ("false", False)]),
        ParameterSpec("draw_array_pads", "Array Pads", "Pad", "Options", 1, kind="choice", choices=[("true", True), ("false", False)]),
        ParameterSpec("note_text_enabled", "Note Text", "Txt", "Options", 1, kind="choice", choices=[("true", True), ("false", False)]),
