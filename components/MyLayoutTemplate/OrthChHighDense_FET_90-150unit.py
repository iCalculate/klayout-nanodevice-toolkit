...
class NewDevice:
    ...
    def __init__(self, layout=None, **kwargs):
        ...
        self.device_width = 150.0   # Device area width (X direction, horizontal, μm)
        self.device_height = 90.0   # Device area height (Y direction, vertical, μm)
        self.mark_margin = kwargs.get('mark_margin', 0.0)           # Mark center distance from boundary (μm)
        
        # ===== Mark Parameters =====
        self.mark_size = kwargs.get('mark_size', 20.0)              # Mark size (μm)
        self.mark_width = kwargs.get('mark_width', 2.0)             # Mark line width (μm)
        
        # ===== Electrode Parameters =====
        # Source electrode parameters
        self.source_outer_x = -47.5      # Outer pad X coordinate
        self.source_outer_y = 5.0        # Outer pad Y coordinate
        self.source_outer_width = 60.0   # Outer pad width (actual drawing horizontal)
        self.source_outer_length = 35.0  # Outer pad length (actual drawing vertical)
        self.source_inner_x = -2.5       # Inner pad X coordinate
        self.source_inner_y_offset = 2.0 # Inner pad Y offset (relative to ch_width)
        self.source_inner_width = 4.0    # Inner pad width (actual drawing horizontal)
        self.source_inner_length = 35.0  # Inner pad length (actual drawing vertical)
        
        # Drain electrode parameters
        self.drain_outer_x = 47.5        # Outer pad X coordinate
        self.drain_outer_y = -5.0        # Outer pad Y coordinate
        self.drain_outer_width = 60.0    # Outer pad width (actual drawing horizontal)
        self.drain_outer_length = 35.0   # Outer pad length (actual drawing vertical)
        self.drain_inner_x = 2.5         # Inner pad X coordinate
        self.drain_inner_y_offset = 2.0  # Inner pad Y offset (relative to ch_width)
        self.drain_inner_width = 4.0     # Inner pad width (actual drawing horizontal)
        self.drain_inner_length = 35.0   # Inner pad length (actual drawing vertical)
        
        # ===== Labeling Parameters =====
        self.label_size = kwargs.get('label_size', 20.0)           # Label size (μm)
        ...
        self.label_offset_x = kwargs.get('label_offset_x', -62.0)    # Label position X offset (μm)
        self.label_offset_y = kwargs.get('label_offset_y', -3.0)  # Label position Y offset (μm)
    ...
    def create_gate_electrode(self, cell, x=0.0, y=0.0):
        ...
        gate_part1_width = self.ch_width + 3.0
        gate_part1_length = self.ch_len / 2.0 + 54.0
        gate_part1_x = x
        ...
        # Part 2: 50μm×40μm hexagon with chamfered corners
        gate_part2_width = 50.0
        gate_part2_height = 40.0
        gate_part2_x = x
        ...
        # Reference line: (-15, -ch_length/2-4) to (30, -35)
        ref_start_x = -15.0
        ref_start_y = -self.ch_len/2.0 - 4.0
        ref_end_x = 30.0
        ref_end_y = -35.0
        ...
        # Chamfer line is parallel to reference line, 12μm below it
        chamfer_scale = 1.0
        chamfer_distance = 30.0
    ...
    def create_channel_material(self, cell, x=0.0, y=0.0):
        ...
        # Channel material rectangle - width is ch_width (X direction, horizontal), length is ch_len+15 (Y direction, vertical)
    ...
    def create_dielectric_layer(self, cell, x=0.0, y=0.0):
        ...
        # 2. Generate source outer pad window (4μm smaller than pad in both dimensions)
        source_window_rect = GeometryUtils.create_rectangle(
            x + self.source_outer_x, y + self.source_outer_y,
            self.source_outer_length - 4.0,
            self.source_outer_width - 4.0,
            center=True
        )
        source_window_region = Region(source_window_rect)
        
        # 3. Generate drain outer pad window (4μm smaller than pad in both dimensions)
        drain_window_rect = GeometryUtils.create_rectangle(
            x + self.drain_outer_x, y + self.drain_outer_y,
            self.drain_outer_length - 4.0,
            self.drain_outer_width - 4.0,
            center=True
        )
...
def main():
    ...
    device_array = device.create_device_array(rows=55, cols=33)
    ...
    param_ranges = {
        'ch_len': [0.5, 20.0, 55],      # Row scan: channel length from 0.5μm to 20μm, 55 values
        'ch_width': [1.0, 20.0, 33],    # Column scan: channel width from 1μm to 20μm, 33 values
    }
    
    # No offset, create scan array directly
    scan_array = device.scan_parameters_and_create_array(param_ranges, rows=55, cols=33, offset_x=0, offset_y=0)
    print(f"Parameter scan array created: {scan_array.name}")
    
    
    # Save layout file
    output_file = "TEST_NEW_DEVICE.gds"
